    ...
    try {
        ...
        if(isVisitor.length>0){
            ...
            if(password===result.Password){
                await db.query(`
                    update Visitor set EntryTime = ${moCurrentTime} where Password = '${password}'
                `)
                return res.json({
            ...
            if(password===result.Password){
                await db.query(`
                    update GrabDeliver set EntryTime = ${moCurrentTime} where Password = '${password}'
                `)
                return res.json({
                    message: "Validation successfully"
                });
            ...
            if(password===result.Password){
                await db.query(`
                    update HouseOwnerTime set EntryTime = ${moCurrentTime} where Password = '${password}'
                `)
                return res.json({
                    message: "Validation successfully"
                });
